...
console.log('Hello from verify-phone endpoint!')
...
serve(async (req) => {
  ...
  try {
    ...
    if (!token || !phoneNumber) {
      throw new Error('Verification token and phone number are required')
    }
    ...
    const { data, error } = await supabaseClient.rpc('verify_phone', {
      p_token: token,
    ...
    await supabaseClient.rpc('log_security_event', {
      ...
      p_details: { token: token, phone_number: phoneNumber }
    })
